setwd("E:/spleen_alldata/NT_spleen_TC/tumor_STC")
...
DESeq2_result$Group[which((DESeq2_result$p_val_adj<0.05) & (DESeq2_result$avg_log2FC>0.25))] <- 'highly expressed in T-ko'
DESeq2_result$Group[which((DESeq2_result$p_val_adj<0.05) & (DESeq2_result$avg_log2FC< (-0.25)))] <- 'highly expressed in T-wt'
table(DESeq2_result$Group)

DESeq2_result$Label <- ''
DESeq2_result <- DESeq2_result[order(DESeq2_result$avg_log2FC,decreasing = TRUE),]
up_genes <- head(rownames(DESeq2_result)[which(DESeq2_result$Group=='highly expressed in T-ko')],10)
down_genes <- tail(rownames(DESeq2_result)[which(DESeq2_result$Group=='highly expressed in T-wt')],10)
...
plot2      
ggsave('colon_fob_alldeg_BP.pdf',plot2,width = 12,height = 6)
...
write.csv(sig_dge.down,'TS_Fob_dge_wt.csv')
...
plot2 <- ggplot(datbar,aes(x=Count,y=Term,fill=p.adjust))+
  geom_bar(stat = 'identity',mapping = aes(fill=p.adjust))+
  theme_test()+
  scale_colour_gradient(low="#abddff",high="#5066a1",aesthetics = "fill")+
  theme(axis.text = element_text(face='bold',size = 8,angle = 0,family = 'sans',colour = 'black'),
        legend.position = c(0.9,0.5))+
  xlab('T_ko up_regulated')+ylab('')+
  ggtitle("Naive Cd8+ TC GO Term")
  
plot2      
ggsave('Naive Cd8_BP_ko.pdf',plot2,width = 8,height = 6)
...
write.csv(egoALL_wt, 'TS_Fob_enrichGO_all_wt.csv')
